...
uint16_t noCommLoops = 0;                 //main loop without communication counter
...
const int PIN_SIDE_LIGHT_LED = 25;                  //Side light blinking led pin

unsigned long lastMilli = 0;

//l298n driver pin configuration
const int A_motor_speed = 2;
const int A_motor_1 = 4;
const int A_motor_2 = 3;  

const int B_motor_speed = 6;
const int B_motor_1 = 8;
const int B_motor_2 = 7; 
...
const double speed_to_pwm_ratio = 0.00335;    //Ratio to convert speed (in m/s) to PWM value. It was obtained by plotting the wheel speed in relation to the PWM motor command (the value is the slope of the linear function).
const double min_speed_cmd = 0.1273;          //(min_speed_cmd/speed_to_pwm_ratio) is the minimum command value needed for the motor to start moving. This value was obtained by plotting the wheel speed in relation to the PWM motor command (the value is the constant of the linear function).
...
const double max_speed = 8.5;                 //Max speed in m/s

int PWM_leftMotor = 0;                     //PWM command for left motor
int PWM_rightMotor = 0;                    //PWM command for right motor 
                                                      
// PID Parameters
const double PID_left_param[] = { 0.3, 0.015, 0.15 }; //Respectively Kp, Ki and Kd for left motor PID
const double PID_right_param[] = { 0.3, 0.015, 0.15 }; //Respectively Kp, Ki and Kd for right motor PID
...
void setup() {
  ...
  attachInterrupt(PIN_ENCOD_A_MOTOR_LEFT, encoderLeftMotor, RISING);
  ...
  attachInterrupt(PIN_ENCOD_A_MOTOR_RIGHT, encoderRightMotor, RISING);
}
...
void loop() {
  ...
  {                                                                           // enter timed loop
    ...
    PWM_leftMotor = constrain(((speed_req_left+sgn(speed_req_left)*min_speed_cmd)/speed_to_pwm_ratio) + (speed_cmd_left/speed_to_pwm_ratio), -115, 100); //
    ...
    PWM_rightMotor = constrain(((speed_req_right+sgn(speed_req_right)*min_speed_cmd)/speed_to_pwm_ratio) + (speed_cmd_right/speed_to_pwm_ratio), -110, 110); // 
    ...
    publishSpeed(10);   //Publish odometry on ROS topic
  }
 }
